...
def build_darcy2(coef, F):
    ...
    # X1, Y1 = np.meshgrid(np.arange(1/(2*K), 1, 1/K), np.arange(1/(2*K), 1, 1/K))
    # X2, Y2 = np.meshgrid(np.concatenate((np.arange(0, 1, 1/(K-1)),np.ones(1))), np.concatenate((np.arange(0, 1, 1/(K-1)),np.ones(1))))
...
if __name__ == "__main__":
    ...
    # w0 = np.exp(GRF.sample(1).numpy()[0])
    # # w0 = np.ones([s,s])
    # u = solve_darcy1(w0, f)
    # plt.figure()
    # plt.imshow(w0)
    ...
    if style == "GRF":
        fset = np.exp(GRF.sample(usample))
    ...
    # reader = torch.load('darcy_train_102_fGRF.pt')
    ...
    for i in range(num_examples):
        ...
        if style == "GRF":
            noise = (np.exp(GRF_noise.sample(1))*np.min(u)*0.05/edge).numpy()
        elif style == "cheb":
            noise = ((np.exp(Chebyshev_truncation_polynomial(edge, cheby_num,1))*np.min(u)*0.05/edge).reshape(edge,edge)).numpy()
        ...
        f = A@u
        t3 = time.time()
        f = np.matrix(f.reshape(edge,edge))
        y_hat = torch.tensor(ori_solver).reshape(edge,edge)
